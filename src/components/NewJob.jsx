        ...
        <div style={S.card}>
          ...
          <div style={{ marginBottom: 18 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {[{ value: "automotive", label: "Automotive" }, { value: "lift_truck", label: "Lift Truck" }].map(opt => (
                <button key={opt.value} onClick={() => handleJobTypeChange(opt.value)} style={{
                  flex: 1, padding: "10px 8px",
                  background: jobType === opt.value ? C.accent : C.elevated,
                  border: `1px solid ${jobType === opt.value ? C.accent : C.border}`,
                  borderRadius: 8, color: jobType === opt.value ? "#fff" : C.textSecondary,
          ...
          {custMode === "existing" ? (
            <div style={{ position: "relative", marginBottom: 14 }}>
              ...
              <input style={{ ...S.input, width: "100%", boxSizing: "border-box" }}
                placeholder="Type name or phone..." value={custSearch}
                onChange={e => { setCustSearch(e.target.value); setCustSearchActive(true); if (!e.target.value) setSelectedCustomer(""); }}
                onFocus={() => setCustSearchActive(true)} />
              {custSearchActive && custSuggestions.length > 0 && (
                <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, zIndex: 100, boxShadow: "0 4px 16px rgba(0,0,0,0.18)", overflow: "hidden" }}>
                  {custSuggestions.map(c => {
                    const cvs = vehicles.filter(v => (v.customer_id === c.id || v.customerId === c.id) && (v.job_type || "automotive") === jobType);
                    return (
                      <div key={c.id} onMouseDown={() => selectCustomerFromSearch(c)} onTouchStart={() => selectCustomerFromSearch(c)}
                        ...
                        <div style={{ fontSize: 11, color: C.textMuted, marginTop: 2 }}>
                          {c.phone}{cvs.length > 0 ? `  ·  ${cvs.map(v => v.job_type === "lift_truck" ? `${v.make} ${v.model}` : `${v.year} ${v.make} ${v.model}`).join(", ")}` : ""}
                        </div>
                      </div>
                    );
                  })}
              ...
              {selectedCustomer && (
                <div style={{ marginTop: 8, padding: "8px 12px", background: C.elevated, borderRadius: 6, borderLeft: `3px solid ${C.accent}` }}>
                  <div style={{ fontSize: 12, color: C.accent, fontWeight: 600 }}>✓ {customers.find(c => c.id === selectedCustomer)?.name}</div>
                  <div style={{ fontSize: 11, color: C.textMuted }}>{customers.find(c => c.id === selectedCustomer)?.phone}</div>
                </div>
              )}
            </div>
          ) : (
      ...
      {step === 1 && (
        <div style={S.card}>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
              {["existing", "new"].map(m => (
                <button key={m} onClick={() => setVehMode(m)}
                  ...
                  {m === "existing" ? `Existing ${isLiftTruck ? "Lift Truck" : "Vehicle"}` : `New ${isLiftTruck ? "Lift Truck" : "Vehicle"}`}
                </button>
              ))}
            </div>
          )}
          {(vehMode === "existing" && custVehicles.length > 0) ? (
            <Input label={`Select ${isLiftTruck ? "Lift Truck" : "Vehicle"}`} as="select" value={selectedVehicle} onChange={e => setSelectedVehicle(e.target.value)}>
              <option value="">Choose {isLiftTruck ? "lift truck" : "vehicle"}...</option>
              {custVehicles.map(v => (
                <option key={v.id} value={v.id}>
                  {isLiftTruck ? `${v.make} ${v.model}${v.serial ? ` · S/N ${v.serial}` : ""}` : `${v.year} ${v.make} ${v.model}`}
                </option>
              ))}
            </Input>
          ) : isLiftTruck ? (
          ...
          {isLiftTruck && vehMode === "existing" && selectedVehicle && (
            <Input label="Current Hour Meter Reading" type="number" inputMode="decimal" value={newEquip.hour_meter} onChange={e => setNewEquip({ ...newEquip, hour_meter: e.target.value })} placeholder="e.g. 4500" />
          )}
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {["Cash", "Card", "Venmo", "Zelle"].map(m => (
                <button key={m} onClick={() => setPayMethod(m)} style={{
                  ...
                  borderRadius: 6, color: payMethod === m ? "#fff" : C.textSecondary,
